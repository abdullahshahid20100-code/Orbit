...
export const AuthModal: React.FC<AuthModalProps> = ({
  ...
  const availableUsers = store.getUsers();
  ...
  const handleLoginSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    setLoginError(null);
    const res = store.login(loginUsername, loginPassword);
    if (res.success && res.user) {
      onSuccess(res.user);
      if (onClose) onClose();
    } else {
      setLoginError(res.error || 'Login failed. Please check username and password.');
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/90 backdrop-blur-md flex items-center justify-center p-4 animate-in fade-in duration-200">
      <div className="w-full max-w-md bg-[#10131d] border border-slate-800 rounded-3xl overflow-hidden shadow-2xl p-6 relative flex flex-col max-h-[95vh] overflow-y-auto no-scrollbar">
        {/* Close Button if not strictly initial or if any user exists */}
        {(!isFirstTime || availableUsers.length > 0) && (
          <button
            onClick={() => {
              if (onClose) onClose();
              else {
                const cur = store.getCurrentUser();
                if (cur) onSuccess(cur);
              }
            }}
        ...
        {/* Tabs for Signup / Login (when not editing existing profile) */}
        {!isEditing && (
          <div className="flex rounded-2xl bg-slate-900/90 p-1 border border-slate-800 mb-4">
            <button
              type="button"
              onClick={() => {
                setAuthMode('signup');
        ...
        {/* Log In Form */}
        {!isEditing && authMode === 'login' ? (
          <form onSubmit={handleLoginSubmit} className="space-y-3.5 text-xs">
            {/* Quick account switcher if accounts are present */}
                ...
                <div className="space-y-1.5">
                  {availableUsers.map((u) => (
                    <button
                      ...
                      className="w-full flex items-center gap-2.5 p-2 rounded-xl bg-slate-900 hover:bg-slate-800 border border-slate-800 hover:border-indigo-500/50 transition-colors text-left"
                    >
                      <img
                        src={u.avatar}
                        alt={u.displayName}
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-[10px] text-indigo-400 font-medium truncate">@{u.username}</p>
                      </div>
                      <span className="text-[10px] px-2 py-1 rounded-lg bg-indigo-600/30 text-indigo-300 font-semibold">
                        Select
                      </span>
                    </button>
                  ))}
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  type="text"
                  required
                  placeholder="e.g. abdullahshahid"
                  value={loginUsername}
                  onChange={(e) =>
                    setLoginUsername(e.target.value.toLowerCase().replace(/\s+/g, ''))
                  }
                  className="w-full bg-[#181c28] border border-slate-700/80 rounded-xl pl-8 pr-3.5 py-2.5 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-indigo-500"
                />
              </div>
            </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full bg-[#181c28] border border-slate-700/80 rounded-xl pl-9 pr-10 py-2.5 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-indigo-500"
                />
                <button
                  type="button"
            ...
            <div className="pt-2">
              <button
                ...
                className="w-full py-2.5 rounded-2xl bg-gradient-to-r from-indigo-600 via-indigo-500 to-purple-600 hover:opacity-95 text-white text-xs font-bold shadow-[0_0_16px_rgba(99,102,241,0.4)] disabled:opacity-40 transition-all flex items-center justify-center gap-1.5"
              >
          ...
          <form onSubmit={handleSubmit} className="space-y-3.5 text-xs">
                ...
                <div className="mt-1 text-[11px]">
                  {validationState.valid ? (
                    ...
                    <div className="flex flex-col gap-1">
                      ...
                      {validationState.error.includes('already taken') && (
                        <button
                          type="button"
                          onClick={() => {
                          ...
                          className="text-left text-indigo-400 underline hover:text-indigo-300 font-medium"
                        >
                          Is this your account? Click here to Log In instead →
                        </button>
                      )}
                    </div>
                  ) : null}
            ...
            {/* 5. Profile Picture (DP) from Gallery */}
            <div>
              <div className="flex items-center justify-between mb-1">
                <label className="text-xs font-semibold text-slate-300">
